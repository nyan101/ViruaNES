...
void	PPU::SetRenderScanline( INT scanline )
{
	ScanlineNo = scanline;
	if( scanline < 240 ) {
		lpScanline = lpScreen+(256+16)*scanline;
	}
}

void	PPU::ScanlineStart()//첫 scanline(주사선)읽기
{
...
void	PPU::ScanlineNext()//다음 scanline읽기
{
	if( PPUREG[1] & (PPU_BGDISP_BIT|PPU_SPDISP_BIT) ) {
...
BYTE	BGmono[33+1];
	...
	::memset( BGmono, 0, sizeof(BGwrite) );

	// Render BG
	if( !(PPUREG[1]&PPU_BGDISP_BIT) ) {
		::memset( lpScanline, BGPAL[0], 256+16 );
			...
			if( !bExtLatch ) {
				...
				// Monocrome effect (for Final Fantasy)
					...
					for( INT i = 0; i < 33; i++ ) {
						...
						mono = (PPUREG[1]&PPU_COLORMODE_BIT)<<6;
						BGmono[i] = mono;

						attr = ((pNTBL[attradr+(ntbl_x>>2)]>>((ntbl_x&2)+attrsft))&3)<<2;

						if( cache_tile != tileadr || cache_attr != attr || cache_mono != mono ) {
							cache_tile = tileadr;
							cache_attr = attr;
							cache_mono = mono;
							...
							*pBGw = chr_l|chr_h;
							...
							register INT	c1 = ((chr_l>>1)&0x55)|(chr_h&0xAA);
							register INT	c2 = (chr_l&0x55)|((chr_h<<1)&0xAA);
							pScn[0] = pBGPAL[c1>>6]		|mono;
							pScn[4] = pBGPAL[(c1>>2)&3]	|mono;
							pScn[1] = pBGPAL[c2>>6]		|mono;
							pScn[5] = pBGPAL[(c2>>2)&3]	|mono;
							pScn[2] = pBGPAL[(c1>>4)&3]	|mono;
							pScn[6] = pBGPAL[c1&3]		|mono;
							pScn[3] = pBGPAL[(c2>>4)&3]	|mono;
							pScn[7] = pBGPAL[c2&3]		|mono;
							}
						} else {
							*(DWORD*)(pScn+0) = *(DWORD*)(pScn-8);
							*(DWORD*)(pScn+4) = *(DWORD*)(pScn-4);
